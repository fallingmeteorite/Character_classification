...
def Thread_calls(process_src, process_number, confidence, body_cut):
    ...
    for n in range(1, num_file + 1):
        new_file = os.path.join("Cache\\" + str(n))
    ...
    for index in range(int(cnt)):
        _processes = (Thread(target=start_processes, args=(str(index + 1), confidence, body_cut)))
        _processes.start()


# 开启线程处理文件
def start_processes(index, confidence, body_cut):
    time1 = time.time()

    input_work_path = (f'Cache\\{index}')

    run_classification(input_work_path, confidence, body_cut)
